...
namespace inventario_api.Repository
{
    public class Queries
    {
        ...
        public static string Articulos = "Select A.ARTICULOID,A.CODIGO,A.DESCRIPCION from ARTICULOTIPOINVENTARIO ATI INNER JOIN ARTICULO A ON ATI.ARTICULOID = A.ARTICULOID WHERE ATI.TIPOINVENTARIOID = @TIPOINVENTARIOID AND ATI.AREAID = @AREAID AND A.HABILITADO = 1 AND ATI.HABILITADO = 1 ORDER BY A.DESCRIPCION";
        public static string CrearInventarioCabecera = "INSERT INTO INVENTARIOCABECERA(FECHACREACION,ESTADO,ARCHIVOSTOCK,USUARIOID,TIPOINVENTARIOID,AREAID,FECHAINICIO,FECHAVISUALIZACION) VALUES (GETDATE(),@ESTADO,@ARCHIVOSTOCK,@USUARIOID,@TIPOINVENTARIOID,@AREAID,@FECHAINICIO,@FECHAVISUALIZACION)";
        ...
        //public static string ValidarInventarioAbierto = "SELECT A.AREAID,A.DESCRIPCION FROM INVENTARIOCABECERA I INNER JOIN AREA A ON I.AREAID = A.AREAID AND I.AREAID = @AREAID and convert(nvarchar(10),I.fechainicio,23) = convert(nvarchar(10),@fechainicio,23) and I.tipoinventarioid = @tipoinventarioid AND I.ESTADO = 'APERTURADO'";

        //Toma
        public static string InventariosAbiertos = "select ic.INVENTARIOID,IC.FECHAINICIO,IC.FECHAVISUALIZACION,ISNULL(IC.FECHAFIN,GETDATE()) AS FECHAFIN,IC.TIPOINVENTARIOID,IC.AREAID,AL.DESCRIPCION AS ALMACEN,a.DESCRIPCION as AREA, ti.DESCRIPCION as TIPOINVENTARIO,ic.ARCHIVOSTOCK from INVENTARIOCABECERA ic inner join USUARIOAREA ua ON ua.AREAID = ic.AREAID inner join TIPOINVENTARIO ti on ic.TIPOINVENTARIOID = ti.TIPOINVENTARIOID INNER JOIN AREA A ON IC.AREAID = A.AREAID INNER JOIN ALMACEN AL ON A.ALMACENID = AL.ALMACENID where ua.USUARIOID = @USUARIOID and IC.ESTADO = 'APERTURADO' and IC.FECHAVISUALIZACION <= GETDATE() ";
        public static string InventariosCerrados = "select top(10) ic.INVENTARIOID,ISNULL(IC.FECHAVISUALIZACION,GETDATE()) AS FECHAVISUALIZACION,IC.FECHAINICIO,ISNULL(IC.FECHAFIN,GETDATE()) AS FECHAFIN,IC.TIPOINVENTARIOID,IC.AREAID,AL.DESCRIPCION AS ALMACEN,a.DESCRIPCION as AREA, ti.DESCRIPCION as TIPOINVENTARIO,ic.ARCHIVOSTOCK from INVENTARIOCABECERA ic inner join USUARIOAREA ua ON ua.AREAID = ic.AREAID inner join TIPOINVENTARIO ti on ic.TIPOINVENTARIOID = ti.TIPOINVENTARIOID INNER JOIN AREA A ON IC.AREAID = A.AREAID INNER JOIN ALMACEN AL ON A.ALMACENID = AL.ALMACENID where ua.USUARIOID = @USUARIOID and IC.ESTADO = 'FINALIZADO'  order by IC.FECHAFIN desc";
        ...
        public static string ArticulosTipoInventarioMtm = "select at.articuloid,at.areaid,at.tipoinventarioid,at.orden,at.localizacion,isnull(at.habilitado,1) as habilitado,ar.CODIGO,ar.DESCRIPCION as articulo,a.DESCRIPCION as area,al.almacenid,al.DESCRIPCION as almacen,l.localid, l.DESCRIPCION as local,e.empresaid,e.DESCRIPCION as empresa,ti.descripcion as tipoinventario from ARTICULOTIPOINVENTARIO at inner join AREA a on a.AREAID = at.AREAID inner join ALMACEN al on al.ALMACENID = a.ALMACENID inner join LOCAL l on l.LOCALID = al.LOCALID inner join EMPRESA e  on e.EMPRESAID=l.EMPRESAID inner join TIPOINVENTARIO ti on ti.TIPOINVENTARIOID = at.TIPOINVENTARIOID inner join ARTICULO ar on ar.ARTICULOID = at.ARTICULOID where e.su = @su";
        public static string CrearArticuloTipoInventarioMtm = "Insert into ARTICULOTIPOINVENTARIO (ARTICULOID,AREAID,TIPOINVENTARIOID,ORDEN,LOCALIZACION,HABILITADO) values (@ARTICULOID,@AREAID,@TIPOINVENTARIOID,@ORDEN,@LOCALIZACION,@HABILITADO)";
        public static string ActualizarArticuloTipoInventarioMtm = "Update ARTICULOTIPOINVENTARIO set ORDEN = @ORDEN,LOCALIZACION=@LOCALIZACION,HABILITADO = @HABILITADO where ARTICULOID = @ARTICULOID AND AREAID = @AREAID AND TIPOINVENTARIOID = @TIPOINVENTARIOID ";
        ...
        //Toma de inventario consolidada
        public static string TomaConsolidadaCabecera = "USP_CONSOLIDARTOMAINVENTARIO";
        public static string TomaConsolidadaDetalle = "USP_CONSOLIDARTOMAINVENTARIODETALLE";

        //Registrar Ingresos / Salidas
        public static string VerificarRegistroMovimiento = "USP_VerificarRegistroMovimiento"; //@tipoMovimiento nvarchar(50),@areaId int,@fecha nvarchar(50)
        public static string IniciarRegistroMovimiento = "USP_IniciarRegistroMovimiento";//@tipoMovimiento nvarchar(50),@areaId int,@fecha nvarchar(50)
        public static string TraerMovimientoDetalle = "SELECT MD.*,A.CODIGO,A.DESCRIPCION AS ARTICULO,UM.DESCRIPCION AS UNIDADMEDIDA FROM MOVIMIENTODETALLE md INNER JOIN ARTICULO A ON MD.ARTICULOID = A.ARTICULOID INNER JOIN UNIDADMEDIDA UM ON UM.UNIDADMEDIDAID = A.UNIDADMEDIDAID WHERE MD.MOVIMIENTOID = @MOVIMIENTOID;";
        public static string GuardarMovimientoDetalle = "UPDATE MovimientoDetalle SET CANTIDAD=@CANTIDAD,PRECIOUNITARIO = @PRECIOUNITARIO WHERE MOVIMIENTOID = @MOVIMIENTOID AND ARTICULOID = @ARTICULOID ";

        public static string TraerIngresoSalidaConsolidada = "USP_TraerIngresoSalidaConsolidada";
    }
}
