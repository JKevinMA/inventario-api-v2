...
namespace inventario_api.Repository
{
    public class Queries
    {
        ...
        public static string Articulos = "Select A.ARTICULOID,A.CODIGO,A.DESCRIPCION from ARTICULOTIPOINVENTARIO ATI INNER JOIN ARTICULO A ON ATI.ARTICULOID = A.ARTICULOID WHERE ATI.TIPOINVENTARIOID = @TIPOINVENTARIOID AND ATI.AREAID = @AREAID ORDER BY A.DESCRIPCION";
        ...
        public static string ObtenerInventarioDetalle = "SELECT  ID.INVENTARIOID,A.ARTICULOID,a.CODIGO, A.DESCRIPCION AS ARTICULO,ID.STOCKTEORICO,ID.CANTIDAD,id.PrecioPromedio,(ID.STOCKTEORICO-ID.CANTIDAD) AS FALTANTE ,ABS((ID.STOCKTEORICO-ID.CANTIDAD)*ID.PRECIOPROMEDIO) AS ABSVALDIF FROM INVENTARIODETALLE ID INNER JOIN ARTICULO A ON ID.ARTICULOID = A.ARTICULOID WHERE ID.INVENTARIOID = @INVENTARIOID ";
